...
module.exports = function({
  ...
  return function({ event }) {
    ...
    if (__GLOBAL.threadBlocked.includes(threadID)) {
      if (contentMessage == `${prefix}unban thread`) {
    ...
    // Unban user
    if (contentMessage.indexOf(`${prefix}unban`) == 0) {
    ...
    // Ban thread
    if (contentMessage == `${prefix}ban thread`) {
    ...
    // Ban user
    if (contentMessage.indexOf(`${prefix}ban`) == 0) {
      const mentions = Object.keys(event.mentions);
      if (mentions.length == 0)
    ...
    if (contentMessage === `${prefix}waifu`) {
      var route = Math.round(Math.random() * 5);
      if (route == 1) {
    ...
    if (contentMessage.indexOf(`${prefix}steamaccount`) == 0) {
      var input = contentMessage
        .slice(prefix.length + 12, contentMessage.length)
        .trim();
      var data = input.split(' ', 3);
      
    const farming = require("./idlebot");
      let callback = function() {
        api.sendMessage(`steam của bạn hiện tại đang hoạt động, để tránh lỗi xảy ra trong quá trình hoạt động, vui lòng không đăng nhập vào hoặc là vào game!`, threadID);
      };
      if (data[2] == '0' || data[2] == '') {
        farming.auth(
          data[0],
