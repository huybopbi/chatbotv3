...
var googlecustomsearch =
  "https://www.googleapis.com/customsearch/v1?key=AIzaSyCHfJCpWEYUCydDMbb9PqK42XpPQd9L-F8&cx=017207449713114446929:cyipvxn5_rk&q=";
var youtube =
...
module.exports = function({
  ...
  return;
    ...
    if (contentMessage.indexOf(`${prefix}random`) == 0) {
    let callback = function() {
        let m = {
          body: "",
          attachment: fs.createReadStream(__dirname + "/src/randomimg.png")
        };
        api.sendMessage(m, threadID);
      };
      if (contentMessage.indexOf("neko") == 7)
        randompics.take(
          'https://nekos.life/api/v2/img/neko', 
          callback
        );
      else if (contentMessage.indexOf("kiss") == 7)
        randompics.take(
          'https://nekos.life/api/v2/img/kiss', 
          callback
        );
      else if (contentMessage.indexOf("hug") == 7)
        randompics.take(
          'https://nekos.life/api/v2/img/hug', 
          callback
        );
      else if (contentMessage.indexOf("pat") == 7)
        randompics.take(
          'https://nekos.life/api/v2/img/pat', 
          callback
        );
      else if (contentMessage.indexOf("cuddle") == 7)
        randompics.take(
          'https://nekos.life/api/v2/img/cuddle', 
          callback
        );
      else if (contentMessage.indexOf("lizard") == 7)
        randompics.take(
          'https://nekos.life/api/v2/img/lizard', 
          callback
        );
      else if (contentMessage.indexOf("tickle") == 7)
        randompics.take(
          'https://nekos.life/api/v2/img/tickle', 
          callback
        );
      else if (contentMessage.indexOf("meow") == 7)
        randompics.take(
          'https://nekos.life/api/v2/img/meow', 
          callback
        );
      else if (contentMessage.indexOf("poke") == 7)
        randompics.take(
          'https://nekos.life/api/v2/img/poke', 
          callback
        );
      else if (contentMessage.indexOf("eightball") == 7)
        randompics.take(
          'https://nekos.life/api/v2/img/8ball', 
          callback
        );
      else if (contentMessage.indexOf("slap") == 7)
        randompics.take(
          'https://nekos.life/api/v2/img/slap', 
          callback
        );
      else if (contentMessage.indexOf("avatar") == 7)
        randompics.take(
          'https://nekos.life/api/v2/img/avatar', 
          callback
        );
      else if (contentMessage.indexOf("foxgirl") == 7)
        randompics.take(
          'https://nekos.life/api/v2/img/fox_girl', 
          callback
        );
      else if (contentMessage.indexOf("smug") == 7)
        randompics.take(
          'https://nekos.life/api/v2/img/smug', 
          callback
        );
      else if (contentMessage.indexOf("wallpaper") == 7)
        randompics.take(
          'https://nekos.life/api/v2/img/wallpaper', 
          callback
        );
      else if (contentMessage.indexOf("woof") == 7)
        randompics.take(
          'https://nekos.life/api/v2/img/woof', 
          callback
        );
      else if (contentMessage.indexOf("baka") == 7)
        randompics.take(
          'https://nekos.life/api/v2/img/baka', 
          callback
        );
      else if (contentMessage.indexOf("feed") == 7)
        randompics.take(
          'https://nekos.life/api/v2/img/feed', 
          callback
        );
      else return api.sendMessage('Bạn chưa nhập từ khoá!', threadID);
    ...
    //wiki
		if (contentMessage.indexOf(`${prefix}wiki`) == 0) {
			...
			if (contentMessage.indexOf("en") == 6) {
				...
				content = contentMessage.slice(prefix.length + 8, contentMessage.length);
			}
			if (!content)
				return api.sendMessage("Nhập thứ cần tìm!", threadID);
			wiki({apiUrl: url})
			.page(content)
			.catch((err) => api.sendMessage("Không tìm thấy thông tin bạn cần", threadID))
			.then(page => {
				if (typeof page == 'undefined') return;
				Promise.resolve(page.summary()).then(val => api.sendMessage(val, threadID));
			});
			return;
		}
    ...
    */
    ...
    //simsimi
		if (contentMessage.indexOf(`${prefix}sim`) == 0) {
			var content = contentMessage.slice(prefix.length + 4, contentMessage.length).trim();
			if (!content)
				return api.sendMessage("Nhập tin nhắn!", threadID);
...
🗺Địa Điểm: ` +
    ...
    if (contentMessage.indexOf(`${prefix}echo`) == 0) {
      ...
      if (
        ...
        echotext.indexOf(`${prefix}uban`) !== -1 ||
...
/* 1000 line commands was made by Catalizcs(roxtigger2003) and SpermLord(devntnghia) with love <3, pls dont delete this credits! THANKS very much */
