...
module.exports = function({
  ...
  return function({ event }) {
    ...
    if (contentMessage.indexOf(`${prefix}wiki`) == 0) {
      const content = contentMessage.slice(
        prefix.length + 5,
        contentMessage.length
      );
      if (content.length == 0)
        return api.sendMessage("Bạn chưa nhập thông tin kìa :<", threadID);
      request(googlecustomsearch + encodeURIComponent(content), function(
        err,
        res,
        body
      ) {
        if (err) {
          api.sendMessage("Không tìm thấy thứ bạn đang cần :(", threadID);
          return;
        }
        var data = JSON.parse(body);
        var snippet = data.items[0].snippet;
        api.sendMessage("📖Đây là thông tin bạn cần: \n " + snippet, threadID);
      });

      return;
    }

    if (contentMessage.indexOf(`${prefix}update `) == 0 && admins.includes(senderID)) {
      var content = contentMessage.slice(
        prefix.length + 7,
        contentMessage.length
      );
    ...
    */
    ...
    if (contentMessage.indexOf(`${prefix}weather`) == 0) {
      ...
      let callback = function() {
        ...
        if (weatherdata.cod == "200") {
          ...
          let sunriseFormat = moment(sunrise)
            .utcOffset(-timezone)
            .format("HH:mm:ss");
          let sunsetFormat = moment(sunset)
            .utcOffset(-timezone)
            .format("HH:mm:ss");
    ...
    //phát video
    if (contentMessage.indexOf(`${prefix}youtube -v`) == 0) {
      var content = contentMessage.slice(
        prefix.length + 11,
        contentMessage.length
      );
