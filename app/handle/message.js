...
module.exports = function({ api, modules, config, __GLOBAL, User, Thread, Rank }) {
  let { prefix, googleSearch ,wolfarmb ,osuAPI, yandex, tenor, admins, steamAPI, ENDPOINT } = config;
/* ================ CronJob ==================== */
...
/* =============== BAN / UNBAN COMMAND =============== */
		...
		for (const item of cmds) {
			if (contentMessage.indexOf(item) == 0) return api.sendMessage("Lệnh này đã bị cấm!", threadID);
		}
		...
		if (contentMessage.indexOf(`${prefix}ban command`) == 0 && admins.includes(senderID)) {
			...
			if (!content)
				return api.sendMessage("Hãy nhập lệnh cần cấm!" ,threadID);
...
/* ================ Staff Commands ==================== */
    ...
    if (contentMessage.indexOf(`${prefix}report`) == 0) {
      ...
      api.sendMessage("Thông tin lỗi của bạn đã được gửi về admin!", threadID, messageID);
      return;
    }
    ...
    if (
      ...
      for (var i = 0; i < Object.keys(event.mentions).length; i++) {
        var content = contentMessage
          ...
          .trim();
        if (content.length == 0)
          return api.sendMessage(`Chưa nhập thông tin kìa bạn eii`, threadID, messageID);
    ...
    if (contentMessage.indexOf(`${prefix}ping url`) == 0) {
      ...
      if (content.length == 0) return api.sendMessage("ip đâu ?", threadID, messageID);
...
/* ==================== Help Commands ================*/
    ...
    if (
      ...
      if (string.length == 0)
        return api.sendMessage("error: content Not Found!", threadID, messageID);
    ...
    if (contentMessage.indexOf(`${prefix}help`) == 0) {
      ...
      if (content.length == 0)
        return api.sendMessage("Nhập lệnh cần giúp đỡ đi bạn ơi", threadID, messageID);
      ...
      if (content == 'all') {
        ...
        (err, data) => {
          ...
          return api.sendMessage('Đây là toàn bộ lệnh của bot: ' + helpName, threadID, messageID);
        });
        return;
      }
      fs.readFile(
        ...
        (err, data) => {
          if (err)
            return api.sendMessage("Đã xảy ra lỗi không mong muốn!", threadID, messageID);
          var helpMe = JSON.parse(data);
...
/* ==================== General Commands ================ */
    ...
    if (contentMessage.indexOf(`${prefix}wiki`) == 0) {
      ...
      wiki({ apiUrl: url })
        .page(content)
        .catch(err =>
          api.sendMessage("Không tìm thấy thông tin bạn cần", threadID, messageID)
        )
        .then(page => {
    ...
    */
    ...
    if (senderID == threadID)
      return api.sendMessage('Bạn vui lòng add bot vào nhóm để bot hoạt động tối ưu hơn!', threadID, messageID);
...
🗺Địa Điểm: ` +
          ...
          api.sendMessage(`Thông tin thành phố của bạn không đúng!`, threadID, messageID);
          return;
        }
      };
      weather.api(
    ...
    if (contentMessage.indexOf(`${prefix}say`) == 0) {
      ...
      let callback = function() {
        ...
        api.sendMessage(m, threadID, messageID, () => {
    ...
    if (contentMessage.indexOf(`${prefix}choose`) == 0) {
      var input = contentMessage
        ...
        .trim();
      if (input.lenght == 0)
        return api.sendMessage(`Bạn không nhập đủ thông tin kìa :(`, threadID, messageID);
  ...
  `http://api.wolframalpha.com/v2/result?appid=${wolfarm}&i=`;
      ...
      request(wolfram + encodeURIComponent(m), function(err, response, body) {
        if (body.toString() === "Wolfram|Alpha did not understand your input") {
          ...
          api.sendMessage("I Don't understand your question :3 ", threadID, messageID);
          ...
          api.sendMessage("I Don't know how to answer this question", threadID, messageID);
      ...
      return;
		...
		if (contentMessage.indexOf(`${prefix}time`) == 0) {
			...
			if (!content)
				return api.sendMessage("Nhập thành phố!", threadID);
			...
			if (cityname.length == 0 || cityname == '' || timezone.length == 0 || timezone == 0)
				return api.sendMessage("Không tìm thấy " + content, threadID);
			api.sendMessage("Giờ bên " + cityname + " đang là " + moment.tz(timezone).format("HH:mm"), threadID);
			return;
		}
...
/* ==================== NSFW Commands ==================== */
    ...
    //nhentai search
		if (contentMessage.indexOf(`${prefix}nhentai -i`) == 0) {
			...
			if (!id)
				return api.sendMessage("Nhập id!", threadID);
		...
		if (contentMessage.indexOf(`${prefix}hentaivn -i`) == 0) {
			...
			function(error, response, body) {
				fs.writeFile(filename, body, "utf-8", (err) => {
					...
					if (name.length == 0 || tags.length == 0 || link == '/tim-kiem-truyen. html') {
						fs.unlinkSync(filename);
						return api.sendMessage("Không tìm thấy truyện này", threadID);
					}
...
/* ==================== Game Commands ==================== */
    ...
    if (steamAPI == '' || steamAPI == undefined) return api.sendMessage('Bot chưa có steam api!!', threadID, messageID);
      ...
      if (content.indexOf('csgostat') == 0) {
        ...
        var getURL = baseURL + '/ISteamUserStats/GetUserStatsForGame/v0002/?appid=730&key='+ steamAPI +'&steamid='+ steamID;
      ...
      if (content.indexOf('checkban') == 0) {
        ...
        var getURL = baseURL + '/ISteamUser/GetPlayerBans/v1/?key='+ steamAPI +'&steamids=';
        getURL = encodeURI(getURL);
        request(
        {
            uri: getURL
       ...
       if (body.indexOf('[]') !== -1) return api.sendMessage('Bạn đã nhập sai steamID hoặc steamID không tồn tại',threadID, messageID);
        var data = JSON.parse(body).player[0];
            api.sendMessage(' - Community ban: ' + data.CommunityBanned + '\n - Vac: ' + data.VACBanned + '\n - Số lần bị ban: ' + data.NumberOfVACBans +  '\n - Số lần bị Game ban: ' + data.NumberOfGameBans + '\n - Trade ban: ' + data.EconomyBan'\n - Số ngày sau khi bị ban: ' + data.DaysSinceLastBan, threadID, messageID);
});
        return;
      }
      ...
      if (content.indexOf('getid') == 0) {
        ...
        if (input.length == 0) return api.sendMessage('bạn chưa nhập thông tin', threadID, messageID);
        var getURL = baseURL + '/ISteamUser/ResolveVanityURL/v0001/?key='+ steamAPI +'&vanityurl='+ steamID;
...
/* ==================== Media Commands ==================== */
    ...
    if (contentMessage.indexOf(`${prefix}youtube -p`) == 0) {
      ...
      if (content.length == 0)
        return api.sendMessage(" Bạn chưa nhập link youtube!", threadID, messageID);
    ...
    if (contentMessage.indexOf(`${prefix}youtube -s`) == 0) {
      ...
      var youtube =
  `https://www.googleapis.com/youtube/v3/search?part=snippet&maxResults=25&key=${googleSearch}&q=`;
      if (content.length == 0)
      ...
      request(youtube + encodeURIComponent(content), function(
        ...
        api.sendMessage(`https://www.youtube.com/watch?v=` + link, threadID, messageID);
