...
/* ================ CronJob ==================== */
  ...
  if (!fs.existsSync(__dirname + "/src/listCommands.json")) {
      ...
      fs.writeFile(__dirname + "/src/listCommand.json", push, "utf-8", err => {
        if (err) throw err;
        console.log('System: Tạo file listCommand mới thành công!');
      });
    }
    ...
    if (!fs.existsSync(__dirname + "/src/quotes.json")) {
      request("https://type.fit/api/quotes", (err, response, body) => {
        ...
        fs.writeFile(
          ...
          err => {
            if (err) throw err;
            console.log('System: Tạo file quotes mới thành công!');
          }
        );
      });
    } 
  ...
  fs.readFile(__dirname + "/src/groupID.json", "utf-8", (err, data) => {
    ...
    if (!fs.existsSync(__dirname + "/src/listThread.json")) {
      ...
      fs.writeFile(
        ...
        err => {
          if (err) throw err;
          console.log('System: Tạo file listThread mới thành công!');
        }
      );
    }
    ...
    if (
      ...
      api.getThreadList(100, null, ["INBOX"], function(err, list) {
        ...
        list.forEach(item => {
          ...
          console.log('System: gửi thông báo mới thành công!');
        });
      });
      return;
    }
    ...
    if (contentMessage == `${prefix}getids` && admins.includes(senderID)) {
      ...
      api.getThreadList(100, null, ["INBOX"], function(err, list) {
        ...
        fs.writeFile(
          ...
          err => {
            if (err) throw err;
            console.log('System: Tạo file groupID mới thành công!');
          }
        );
      });
      return;
      ...
      fs.readFile(
        ...
        (err, data) => {
          ...
          api.sendMessage(
            ...
            } \n - group: ${helpMe.find(item => item.name == content).group}`,
            threadID
          );
        }
      );
...
/* ==================== Random Pics ================*/
    ...
    if (contentMessage.indexOf(`${prefix}anime`) == 0) {
      ...
      if (jsonData.sfw.hasOwnProperty(content))
        ...
        return api.sendMessage("sai tag, Dùng !help anime để biết thêm chi tiết!", threadID);
...
/* 1455 line commands was made by Catalizcs(roxtigger2003) and SpermLord(spermlord) with love <3, pls dont delete this credits! THANKS very much */
